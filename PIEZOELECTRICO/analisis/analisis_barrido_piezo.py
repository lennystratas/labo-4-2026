...
R2 = 9.8e3  # Ohm
Rs = R2 * (1 - A) / A
Ls = Rs / (Df * np.pi)  # No debería ser sobre 2 pi? Esto funciona
...
phi_aj = lambda f, C2: fase_C2(f, Rs, Ls, Cs, C2, R2)
p0 = (1e-11,)
popt3, pcov3 = curve_fit(T_aj, frecs, T, p0=p0, sigma=DT)
popt4, pcov4 = curve_fit(phi_aj, frecs, phi_d, p0=p0)
...
ax1.set_yscale("log")
ax1.errorbar(frecs, T, fmt=".", yerr=DT, label="Datos", color="darkblue")
ax1.plot(frecs, T_aj(frecs, *popt3))
ax1.set_ylabel("Transferencia")
ax1.legend(loc=3)
