...
picos, _ = find_peaks(x_deg, distance=785)

fig_p, ax_p = plt.subplots(figsize=(9, 4), sharex=True)
ax_p.errorbar(t, x_deg, fmt="--.", lw=0.7, yerr=Dx_deg, label="Datos")
ax_p.plot(t[picos], x_deg[picos], "o", color="red", ms=6, zorder=10)
sin_hilo = slice(picos[1], picos[2])
...
print(np.diff(t[picos]))
# %% Graficar
fig, ax = plt.subplots(figsize=(9, 4), sharex=True)
ax.errorbar(t, x_deg, color=w["rojo"], fmt="--.", lw=0.7, yerr=Dx_deg, label="Datos")
...
fig.text(
    ...
    color=w["verde"],
    weight="bold",
    bbox=dict(facecolor="#EAEAF2", edgecolor=w["verde"], boxstyle="round,pad=0.5"),
)
...
plt.tight_layout()
plt.show()
fig.savefig("figuras/con_vs_sin_hilo.pdf", bbox_inches="tight")
